document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function fetchAWSRegions() {
        ...
        try {
            ...
            messageElement.style.fontSize = '10px'; // Font size 14px
    ...
    async function fetchExistingKeyPairs(region) {
        try {
            ...
            const data = await response.json();
            alert('Fetching exisiting key Pairs', data);
    ...
    async function createSecurityGroup (sg_name, sg_description, selectedRegion){
        try{
            const response = await fetch(`/create_security_group`, {
                method: 'POST', 
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async function fetchExistingSecurityGroups(selectedRegion){
        try{
            const response = await fetch(`/get-security-groups?region=${selectedRegion}`);
            const data = await response.json();
